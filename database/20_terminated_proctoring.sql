...
-- Drop the existing inline CHECK (PostgreSQL auto-names it enteri_ai_session_status_check)
-- and recreate with the additional status value.
ALTER TABLE enteri_ai_session
  DROP CONSTRAINT IF EXISTS enteri_ai_session_status_check;

ALTER TABLE enteri_ai_session
  ADD CONSTRAINT enteri_ai_session_status_check
