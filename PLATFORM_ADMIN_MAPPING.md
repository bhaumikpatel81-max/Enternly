...
### Feature D module catalog seed list — RESOLVED (implemented in commit 4)

`frontend/index.html`'s full `NAV_DEF` (lines 1951-2054) was read. Core CRUD/pipeline screens (`home, dashboard, requisitions, candidates, interviews, offers, users, module_access, settings, raise_ticket, my_reports, reports, team, sys_health, tickets, sys_logs`) stay always-on baseline product surface, never gated. The seeded `module_catalog` (16 keys) is the 7 existing `DELEGABLE_MODULES` (reused verbatim so the inner/outer gates share key names: `vendors, form_fields, req_approvals, organisation, sla_settings, chain_templates, email_templates`) plus 9 new feature-area keys mapped to real `NAV_DEF` nav ids: `campus_hiring, enteri_ai_tracker, kpi_dashboard, gamification, proctoring_review, hiring_plan, cv_repository, ai_scorecard, no_poach`.

**Outer-gate backend enforcement for the 9 new keys**: `module_access.py::require_tenant_module(key)` is applied via `dependencies=[Depends(...)]` at the `APIRouter(...)` construction for the 9 corresponding router files (`campus_bulk_api.py`, `enteri_ai_api.py`, `kpi_api.py`, `gamification_api.py`, `proctoring_api.py`, `hiring_plan_api.py`, `cv_api.py`, `scorecard_api.py`, `no_poach_api.py`). Four of these files (`enteri_ai_api.py`, `proctoring_api.py`, `campus_bulk_api.py`, and — for one specific route — `cv_api.py`) also serve **public, non-staff traffic** (candidate AI-interview endpoints authenticated by invite token, proctoring webcam/heartbeat endpoints authenticated by session token, a campus-candidate resume-upload endpoint, and `cv_api.py`'s `?token=` query-param fallback used by `viewAuth()` new-tab links) — gating those routers naively would have broken live candidate/proctoring/file-view flows. `require_tenant_module` is therefore designed as a **conditional no-op**: it only inspects an `Authorization: Bearer` header; if none is present (or it doesn't decode as a valid staff token), the dependency does nothing and the route's own auth (invite/session token, or `_cv_auth`'s query-param path) runs exactly as before. It only ever blocks a genuine **staff** request whose tenant has the module disabled — closing the "nav hidden client-side, API still open to a direct staff call" gap for the overwhelming majority of real usage, while leaving one narrow, low-severity residual gap: a staff user who already knows a CV's direct-view URL and calls it via the `?token=` query-param path (bypassing the `Authorization` header entirely) is not blocked by the outer gate on that one route. Flagged here rather than fixed, since closing it would require either duplicating `_cv_auth`'s query-param logic into the gate or reworking that route's auth — not worth the added risk in this pass.
