...
5. **Subscription plan `allowed_modules_json` vs. `tenant_module_config` overlap (Feature D+E).** A subscription plan config carries an `allowed_modules_json` and a tenant also gets its own explicit `tenant_module_config` rows. Proposed: plan's `allowed_modules_json` is applied as the **initial default** when a tenant is assigned that plan (copied into `tenant_module_config`), but `tenant_module_config` remains the actual enforced gate afterward — i.e. plan module lists are a template, not a live constraint. Will implement this default; flag if a live-constraint model (plan strictly caps what a tenant can ever enable) is actually wanted instead.
