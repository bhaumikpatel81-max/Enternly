...
def send_calendar_invite(
    ...
    """
    Send a calendar invite (.ics attached) from the configured SMTP account to
    each recipient. One email per recipient so the candidate never sees the panel list.
...
def schedule_meeting(organizer_email: str, candidate_email: str,
    ...
    """
    Create a calendar invite (.ics over SMTP) from the configured SMTP account
    and email it to the candidate + each panel member. No Google API — meet_link is
...
def send_email(
    ...
    # ── SMTP only (all mail sent from SMTP_USER) ───────────────────────────────
